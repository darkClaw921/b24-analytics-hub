...
- **ResizableChartContainer.tsx** - компонент-обертка для чартов с возможностью изменения размера через drag handles (углы и края) и перемещения через drag за заголовок, автоматически обновляет размер и позицию в БД при изменении
...
9. Пользователь может изменять размер чартов горизонтально и вертикально через drag handles (углы и края), размер автоматически сохраняется в БД и чарт адаптируется под новый размер
10. Пользователь может перемещать чарты по дашборду, перетаскивая их за заголовок, позиция автоматически сохраняется в БД (position_x, position_y)
